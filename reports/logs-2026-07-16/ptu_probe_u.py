...
print(f"probe: cav {ci} n={n} n_tot={n_tot} | sig_bar grid {SIG_BAR_GRID} x u grid {U_GRID} "
      f"x T_top {T_TOP_GRID}", flush=True)
out = {}
for SIG_BAR in SIG_BAR_GRID:
    for T_TOP in T_TOP_GRID:
        for u in U_GRID:
            T = t_of(u, 1.0, 0.0, T_BOT, T_TOP)
            beta = 1.0 / T
            tabs = build_tables_u(u, sig_bar=SIG_BAR)
            ...
            out[(SIG_BAR, T_TOP, u)] = {"id_acc": iacc / max(iatt, 1), "disp_acc": dacc / (EQ_SW * n),
                                         "U_trace": us, "drift_half": drift}
            torch.save(out, "reports/logs-2026-07-16/ptu_probe_u.pt")
            print(f"  sig_bar={SIG_BAR:.2f} T_top={T_TOP} u={u:.2f} T={T:.3f}: "
                  f"identity-acc {iacc/max(iatt,1):.3f} "
