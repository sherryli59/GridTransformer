...
# --- PLAIN SMC baseline (no strata): J islands x M walkers of iid q0 seeds; ESS<0.5 resample;
# suffix+blob mutations; lam=1 displacement polish; readout = POPULATION MEAN q~ (fixed Hocky metric).
J_ISL = 4; M_W = 48
results = {}
...
for ci in range(24):
    ...
    qs = []
    for j in range(J_ISL):
        g = torch.Generator(device=dev).manual_seed(4000 + 31 * ci + j + 7777 * SEED)
        Xb, Sb, _ = m.sample_block_b(xo[None].expand(M_W, n, 3).clone(), so[None].expand(M_W, n).clone(),
                                     allm, bnd, sb, R, gen=g)
        lz, qbar, qsd, emin = stratum_smc(Xb, Sb, bnd, sb, R, n, a, xin, g)
        qs.append(qbar)
        print(f"  cav {ci} isl {j}: qbar {qbar:+.3f} (sd {qsd:.3f}, ess_min {emin:.2f})", flush=True)
    import statistics as _st
    print(f"=== cav {ci} PLAIN-SMC q(R={R}) = {_st.mean(qs):+.3f} +- {_st.pstdev(qs):.3f} over {J_ISL} islands "
          f"({time.time()-t0:.0f}s)  [certified cav1: 0.49+-0.04]", flush=True)
    results[ci] = qs
    torch.save(results, f"reports/logs-2026-07-14/plain_smc_fixedmetric_R{R}_s{SEED}.pt")
    ncav += 1
    if ncav >= 2:
        break
