...
def cluster_energy(xC, pos, cluster_idx, s, L):
    ...
    # mask out (i==j) and the redundant lower triangle BEFORE the pair energy calc: the diagonal has
    # d2i==0 (self-distance) -> inv6 -> inf, and inf * 0 (from the iu mask) is nan, not 0.
    d2i = torch.where(iu, d2i, torch.full_like(d2i, 1e6))
    e_intra = pair_e(d2i, s_cl[:, None, :, None].expand(B, M, k, k),
                      s_cl[:, None, None, :].expand(B, M, k, k)).sum((-1, -2))
