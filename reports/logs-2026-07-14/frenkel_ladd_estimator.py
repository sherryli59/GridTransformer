...
POOL = 4096; BATCH = 512; NQ = 64; QSTEPS = 800
...
OUT = f"reports/logs-2026-07-14/frenkel_ladd_R{R}.pt"
...
def tether_family(x_b, s_in, bnd, sb, xin, gen):
    ...
    lam_vals = []; q_min = None; q_sd = 0.0; ens_min = None
...
for ci in range(24):
    ...
    for fi, ms in enumerate(keep):
        ...
        print(f"  fam{fi} {'REF' if r_['has_ref'] else '   '} U_IS/n {r_['rep_Uis_n']:+.3f} members {len(ms):>3} | "
              f"bF {r_['bF']:+.1f} (teth {r_['bF_teth']:+.1f}) q_b {r_['q_b']:+.3f}+-{r_['q_sd']:.3f} | "
              f"G1 equi {r_['equi']:.2f} G3 tail {r_['tail']:.2f}", flush=True)
        results[(ci, fi)] = {k: v for k, v in r_.items() if k != "ens_min"}
    ...
    print(f"  q(R={R}) FRENKEL-LADD = {q_est:+.3f}  (w = {[round(float(x),3) for x in w.tolist()]}, "
          f"w_ref {wref:.3f}; dbF vs best: {[round(float(b - bFs.min()),1) for b in bFs.tolist()]}) "
          f"({time.time()-t0:.0f}s)", flush=True)
    results[(ci, "final")] = {"q_est": q_est, "w": w.tolist(), "wref": wref}
