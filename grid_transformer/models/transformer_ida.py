...
class GraphormerAR(pl.LightningModule):
    def __init__(
        ...
        use_ida: bool = True,           # Changed from use_edge_bias
        spatial_dim: int = 2,           # Added for IDA
        torus: bool = False,
    ...
    def forward(
        ...
        *,
        ...
        # Base causal mask used for both IDA and the Transformer blocks
        causal_mask_bool = torch.tril(torch.ones(T, T, device=x.device, dtype=torch.bool))

        # 1. Apply Periodic IDA Pre-layer
        if self.use_ida and self.ida is not None and coords is not None:
            x = self.ida(
                x=x,
                coords=coords,
        ...
        # 2. Create standard causal bias for the semantic transformer blocks
        bias = torch.zeros(B, 1, T, T, device=x.device, dtype=x.dtype)
        bias = bias.masked_fill(~causal_mask_bool.unsqueeze(0).unsqueeze(0), float("-inf"))

        # 3. Pass through semantic blocks
