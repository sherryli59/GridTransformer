"""EARLY-BURST q~ test (the payoff run). Insight: redist accepts ~40% on a FRESH alien state but ~0% once ~18
local moves + reweighting harden the population by mid-anneal. So: seed alien (fresh/loose), DWELL at a
productive lambda (0.05) and fire the redistribution burst FIRST on the fresh population (before local moves
compound), THEN run the normal quartic anneal to 1. Measure whether ALIEN-seed q~ finally rises toward the
data bracket. Compare baseline (no redist) vs earlyburst; both seeds. lam05, l=0.368, R=2.0, MB=48, N_try=8."""
...
K = 8; K_R = 12; R = 2.0; N_TRY = 8; MB = 48; T = 20; NCAV = 3; NMUT = 2
BURST_LAM = 0.05; N_DWELL = 2; N_BURST = 6                # dwell rungs at 0.05, redist moves per dwell rung
...
def redist(Xb, Sb, bnd, sb, R, lam, q0, Ecur, allm, U, gen, n_try):
    ...
    def w(Xc, Sc, lqu):
        return ((1 - lam) * m.block_log_prob_b(Xc, Sc, allm, bnd, sb, R) - lam * BETA * energy_b(Xc, Sc, bnd, sb) - lqu,
                m.block_log_prob_b(Xc, Sc, allm, bnd, sb, R) if False else None)
    Xr = Xb.repeat_interleave(n_try, 0); Sr = Sb.repeat_interleave(n_try, 0)
    Yf, Sf, lqf = m.sample_block_b(Xr, Sr, U, bnd, sb, R, gen=gen)
    q0f = m.block_log_prob_b(Yf, Sf, allm, bnd, sb, R); Ef = energy_b(Yf, Sf, bnd, sb)
    lwf = ((1 - lam) * q0f - lam * BETA * Ef - lqf).view(B, n_try); q0f = q0f.view(B, n_try); Ef = Ef.view(B, n_try)
    ...
    lwr = ((1 - lam) * m.block_log_prob_b(Yr, Sr2b, allm, bnd, sb, R) - lam * BETA * energy_b(Yr, Sr2b, bnd, sb) - lqr2).view(B, n_try - 1)
...
def run(mode, seed_mode):
    # schedule: seed(0) -> dwell at BURST_LAM (N_DWELL rungs) -> quartic anneal BURST_LAM..1
    tail = (torch.linspace(0, 1, T + 1, device=dev) ** 4); tail = tail[tail > BURST_LAM]
    lam = torch.cat([torch.tensor([0.0, *([BURST_LAM] * N_DWELL)], device=dev), tail])
    gen = torch.Generator(device=dev).manual_seed(0); ncav = 0; qs = []; bacc = []
    for ci in range(24):
        ...
        for t in range(1, len(lam)):
            ...
            lt = float(lam[t])
            in_burst = (mode == "earlyburst" and t <= 1 + N_DWELL and Kr >= 4)   # dwell rungs = fresh pop
            if in_burst:                                                          # redist FIRST, on the loose state
                for _ in range(N_BURST):
                    U = two_blob(n, a, g, Kr)
                    Xb, Sb, q0, Ecur, af = redist(Xb, Sb, bnd, sb, R, lt, q0, Ecur, allm, U, g, N_TRY); bacc.append(af)
            for _ in range(NMUT):
        ...
        if ncav >= NCAV:
            break
    return st.mean(qs), (st.mean(bacc) if bacc else 0.0)


print(f"=== EARLY-BURST q~ (redist on FRESH pop at lam={BURST_LAM}, then anneal) MB={MB} R={R} ===", flush=True)
print(f"N_dwell={N_DWELL} N_burst={N_BURST} N_try={N_TRY}. prior single-burst 1-island q~=0.03; data bracket ~0.96", flush=True)
print(f"{'mode':>11} {'seed':>6} | {'q~':>7} {'burst_acc':>9} {'sec':>5}", flush=True)
res = {}
for mode in ("baseline", "earlyburst"):
    for seed_mode in ("alien", "data"):
        t0 = time.time(); q, af = run(mode, seed_mode); res[(mode, seed_mode)] = q
        print(f"{mode:>11} {seed_mode:>6} | {q:>7.3f} {af:>9.3f} {time.time()-t0:>5.0f}", flush=True)
print(f"\nALIEN: baseline {res[('baseline','alien')]:.3f} -> earlyburst {res[('earlyburst','alien')]:.3f}  "
      f"(data bracket ~{res[('earlyburst','data')]:.3f}); rise => crossing works", flush=True)
