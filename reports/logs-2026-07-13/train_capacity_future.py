...
(use_future_anchors=True). Same mixed-block objective as the block-cond FT, but through the BATCHED
...
def main():
    ...
    ap.add_argument("--out", default=f"{ART}/ka3d_cavity_ebm3ax_rho115_futanchor_knn24.pt")
    ...
    print(f"STAGE4 future-anchor FT: train={len(train)} held={len(held)} mixture={dict(zip(KINDS, WEIGHTS.tolist()))}", flush=True)
    m = KA3DScaffoldEBMBatched(cat_bins=128, cat_range=2.5, knn_pot=24, use_future_anchors=True).to(dev)
    start = 0
    if a.resume and Path(a.out).exists():
        ...
        print(f"warm {a.warm}: {len(miss)} new (future-anchor), {len(unexp)} unused", flush=True)
    m.use_frame = False
    ...
    for step in range(start, a.steps + 1):
        ...
        if step % a.eval_every == 0:
            ...
            torch.save({"state_dict": m.state_dict(), "step": step, "held": e, "knn_pot": 24,
                        "use_future_anchors": True, "cat_bins": 128, "cat_range": 2.5}, a.out)
            if e["blob8"] < best - 1e-3:
                best = e["blob8"]; stale = 0
                torch.save({"state_dict": m.state_dict(), "step": step, "held": e, "knn_pot": 24,
                            "use_future_anchors": True, "cat_bins": 128, "cat_range": 2.5}, a.out.replace(".pt", "_best.pt"))
            else:
                stale += 1
                if stale >= a.patience:
