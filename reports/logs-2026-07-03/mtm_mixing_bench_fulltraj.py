...
def run_arm(name,do_swap,do_mtm):
    ...
    rec_t=[0.0]; rec_x=[x.clone().cpu()]; rec_tag=[-1]; met=[]           # FULL trajectory: every sweep/move
    def rec(tag):
...
for name,dsw,dmtm in (("swap",True,False),("mtm",False,True),("hybrid",True,True)):
    ...
    out=run_arm(name,dsw,dmtm)
    fn=_os.path.join(ART,f"mtm_traj_{name}.pt")
    torch.save({**out,"pos0":pos0.cpu(),"s0":s0.cpu(),"L":L,"seed":42,"M":M,"budget":BUDGET},fn)
    print(f"saved FULL trajectory {fn}  states={out['x'].shape[0]}  ~{out['x'].numel()*4/1e6:.0f}MB",flush=True)
